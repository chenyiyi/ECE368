...
int main(int argc, char* argv[]){
    ...
    while (task0 != 0 && task1 != 0){
		...
		queue = queue +1;
                ...
                printf("arr0 jin dui ,time:%d, status:%d, a0:%d, a1:%d,d0:%d,d1:%d size0:%d, size1:%d\n", time_line,status,time_arr0,time_arr1,time_dep0,time_dep1,q0->size,q1->size);
            }
            else{
                time_dep0 = time_line + ceil(((-1/servicerate)*(log(1-randf()))));
                time_arr0 = time_line + ceil(((-1/arrivalrate0)*(log(1-randf()))));
                busytime = busytime + time_dep0 - time_line;
                status = 1;
                printf("arr0 ,time:%d, status:%d, a0:%d, a1:%d,d0:%d,d1:%d,size0:%d, size1:%d\n", time_line,status,time_arr0,time_arr1,time_dep0,time_dep1,q0->size,q1->size);
            }
        }
		...
		queue = queue+1;
                ...
                printf("arr1 jin dui,time:%d, status:%d, a0:%d, a1:%d,d0:%d,d1:%d,size0:%d, size1:%d\n", time_line,status,time_arr0,time_arr1,time_dep0,time_dep1,q0->size,q1->size);
            }
            else{
                time_dep1 = time_line + ceil(((-1/servicerate)*(log(1-randf()))));
                time_arr1 = time_line + ceil(((-1/arrivalrate1)*(log(1-randf()))));
                busytime = busytime + time_dep1 - time_line;
                status = 1;
                printf("arr1 ,time:%d, status:%d, a0:%d, a1:%d,d0:%d,d1:%d,size0:%d, size1:%d\n", time_line,status,time_arr0,time_arr1,time_dep0,time_dep1,q0->size,q1->size);
            }
        }
        time_line++;
        printf("rand: %f\n",randf());
